...
- [ ] **Переход на SQLite**
    ...
    - [ ] Создать скрипт `migrate_to_sqlite.py` для переноса данных из CSV в `mlbb_data.db`.
    - [ ] Переписать `data_manager.py` для работы с SQLite.
    - [ ] Проверить, что сайт работает с новой системой данных.
